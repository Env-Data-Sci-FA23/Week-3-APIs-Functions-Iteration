...
```{r}
url_1980 <- "https://irmaservices.nps.gov/v3/rest/stats/total/1980"
raw_data_1980 <- httr::GET(url_1980)
unpacked_data_1980 <- httr::content(raw_data_1980, as = "text", encoding = "UTF-8")
final_data_1980 <- jsonlite::fromJSON(unpacked_data_1980)


url_1999 <- "https://irmaservices.nps.gov/v3/rest/stats/total/1999"
raw_data_1999 <- httr::GET(url_1999)
unpacked_data_1999 <- httr::content(raw_data_1999, as = "text", encoding = "UTF-8")
final_data_1999 <- jsonlite::fromJSON(unpacked_data_1999)


url_2018 <- "https://irmaservices.nps.gov/v3/rest/stats/total/2018"
raw_data_2018 <- httr::GET(url_2018)
...
Rocky_MTN_raw<- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=12&endYear=2021")

Rocky_MTN_unpacked <- httr::content(Rocky_MTN_raw, as = "text", encoding = "UTF-8")

Rocky_MTN_data_final <- jsonlite::fromJSON(Rocky_MTN_unpacked)
Rocky_MTN_data_final
...
unit_visitation <- function(startYear, endYear, startMonth, endMonth, parkUnit){
  ...
  unit_raw_data <- httr::GET(url)
  

  extracted_data <- httr::content(unit_raw_data, as = "text", encoding = "UTF-8")
  

  unit_final_data <- jsonlite::fromJSON(extracted_data)
  

  return(unit_final_data)
}
...
romo_visitation <- unit_visitation(startYear = 2010, endYear = 2021, 
...
# Pulling data for Rocky Mountain National Park from November 1992 to December 2021
romo_visitation <- unit_visitation(startYear = 1992, endYear = 2021, 
                                   startMonth = 11, endMonth = 12, 
                                   parkUnit = "ROMO")

# Pulling data for Everglades National Park from November 1992 to December 2021
ever_visitation <- unit_visitation(startYear = 1992, endYear = 2021, 
                                   startMonth = 11, endMonth = 12, 
                                   parkUnit = "EVER")

# Pulling data for Theodore Roosevelt National Park from November 1992 to December 2021
thro_visitation <- unit_visitation(startYear = 1992, endYear = 2021, 
                                   startMonth = 11, endMonth = 12, 
                                   parkUnit = "THRO")
...
```{r}
unit_visitation <- function(startYear, endYear, parkUnit, startMonth = "01", endMonth = "12") {


  new_url <- paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", 
                    parkUnit, "&startMonth=", startMonth, "&startYear=", startYear, 
                    "&endMonth=", endMonth, "&endYear=", endYear)
  
  # Pull in the data
  raw_data <- httr::GET(url = new_url)
...
# If we don't specify the startMonth and endMonth, it will default to January and December

# Rocky Mountain National Park
romo_visitation <- unit_visitation(startYear = 1980, endYear = 2021, parkUnit = "ROMO")

# Everglades National Park
ever_visitation <- unit_visitation(startYear = 1980, endYear = 2021, parkUnit = "EVER")

# Theodore Roosevelt National Park
thro_visitation <- unit_visitation(startYear = 1980, endYear = 2021, parkUnit = "THRO")
...
park_list <- vector("list", length = length(park_units))


for(i in 1:length(park_units)){
  park_list[[i]] <- unit_visitation(
    startYear = 1980, 
    endYear = 2021, 
    parkUnit = park_units[i]
  )

 #name the list elements after the parks for easier reference

  names(park_list)[i] <- park_units[i]
}


all_parks_data <- dplyr::bind_rows(park_list, .id = "park_unit")


summary(all_parks_data)
...
```{r}
park_data <- map(park_units, ~unit_visitation(parkUnit = .x, startYear = 1980, endYear = 2021))




all_parks_data <- bind_rows(park_data, .id = "park_unit")
