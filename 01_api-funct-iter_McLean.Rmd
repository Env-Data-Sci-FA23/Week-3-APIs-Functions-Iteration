...
```{r}
data1980 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980")

data1980 # lists 'UTF-8'

# convert content to text
unpacked_data1980 <- httr::content(data1980, as = "text", encoding = "UTF-8") 

# parse text from JSON to data frame
final_data1980 <- jsonlite::fromJSON(unpacked_data1980)
...
```{r}
data1999 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1999")

data1999 # lists 'UTF-8'

# convert content to text
unpacked_data1999 <- httr::content(data1999, as = "text", encoding = "UTF-8") 
...
```{r}
data2018 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/2018")

data2018 # lists 'UTF-8'

# convert content to text
unpacked_data2018 <- httr::content(data2018, as = "text", encoding = "UTF-8") 
...
```{r}
dataROMO <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=12&endYear=2021")
...
```{r}
dataROMO # lists 'UTF-8'

# convert content to text
unpacked_dataROMO <- httr::content(dataROMO, as = "text", encoding = "UTF-8")
```
...
raw_data <- httr::GET(url = 
                        # parse out year so that it can be chosen with the "year" argument, using paste0()
                        paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unitCode, "&startMonth=", startMonth, "&startYear=", startYear, "&endMonth=", endMonth, "&endYear=", endYear))
...
```{r}
ROMO <- unit_visitation(unitCode = "ROMO", startMonth = 11, startYear = 1992, endMonth = 12, endYear = 2021)


EVER <- unit_visitation(unitCode = "EVER", startMonth = 11, startYear = 1992, endMonth = 12, endYear = 2021)


THRO <- unit_visitation(unitCode = "THRO", startMonth = 11, startYear = 1992, endMonth = 12, endYear = 2021)
...
```{r}
ROMO1 <- unit_visitation1("ROMO", startYear = 1980, endYear = 2021)

EVER1 <- unit_visitation1("EVER", startYear = 1980, endYear = 2021)

THRO1 <- unit_visitation1("THRO", startYear = 1980, endYear = 2021)
```
...
```{r}
ParkList <- c('ROMO', 'EVER', 'THRO')
```

```{r}
output_floop1 <- vector("list", length = length(ParkList))
```
...
for(i in 1:length(ParkList)){
  ...
  output_floop1[[i]] <-
    unit_visitation1(unitCode = ParkList[i], startYear = "1980", endYear = "2021")
  
}
```

```{r}
summary(output_floop1)
```


```{r}
multi_ParkYears <- dplyr::bind_rows(output_floop1)
```
...
```{r}
output_map1 <- ParkList %>% 
  map(~ unit_visitation1(unitCode = ., startYear = 1980, endYear = 2021))
```

```{r}
identical(output_floop1, output_map1)
```

```{r}
multi_ParkYears1 <- bind_rows(output_map1)
```

```{r}
identical(multi_ParkYears, multi_ParkYears1)
```
