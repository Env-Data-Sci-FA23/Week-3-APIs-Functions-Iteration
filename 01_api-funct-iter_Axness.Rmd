...
```{r}
parkwide_visitation_test <- function (year = 1992){
 ...
 data_1980 <- parkwide_visitation_test(year = 1980)
 
 data_1999 <- parkwide_visitation_test(year = 1999)
 
 data_2018 <- parkwide_visitation_test(year = 2018)
...
```{r}
  ...
  packed_data_2 <-GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=12&endYear=2021"))
  unpacked_data_2 <- httr::content(packed_data_2, as= "text", encoding = "UTF-8")
...
for(i in 1:length(years)) {
  ...
  output_floop[[i]] <- parkwide_visitation_test(year = years[i])

}

multi_years <- output_floop %>%
  bind_rows()
...
```{r, eval = F}
parkwide_visitation <- function(unit,start_Month,start_Year,end_Month, end_Year) {
  ...
  "&endYear=", end_Year))
     ...
     unpacked_data_3 <- httr::content(packed_data_3, as= "text", encoding = "UTF-8")
  final_data_3 <- jsonlite::fromJSON(unpacked_data_3)
  return(years_2)
}
  
  test <- parkwide_visitation(unit="ROMO",
  start_Month="01",
  start_Year="2010",
  end_Month="12",
  end_Year="2021")
...
parkwide_visitation <- function(unit, start_Month, start_Year, end_Month, end_Year) {
  raw_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unit,
                                     "&startMonth=", start_Month,
                                     "&startYear=", start_Year,  # Fixing the '='
                                     "&endMonth=", end_Month,    # Fixing the '='
                                     "&endYear=", end_Year))     # Fixing the '='
  
  unpacked_data <- httr::content(raw_data, as = "text", encoding = "UTF-8")  # Correcting the variable name
  final_data <- jsonlite::fromJSON(unpacked_data)  # Correcting the variable name
  return(final_data)  # Correcting the return value
}
...
# Example usage
test_data <- unit_visitation(unit = "ROMO", 
                             start_Month = "01", 
                             start_Year = "2010", 
                             end_Month = "12", 
                             end_Year = "2021")
...
# Pull in visitation data for each park
romo_data <- unit_visitation(unit = "ROMO", 
                             start_Month = start_Month, 
                             start_Year = start_Year, 
                             end_Month = end_Month, 
                             end_Year = end_Year)

ever_data <- unit_visitation(unit = "EVER", 
                             start_Month = start_Month, 
                             start_Year = start_Year, 
                             end_Month = end_Month, 
                             end_Year = end_Year)

thro_data <- unit_visitation(unit = "THRO", 
                             start_Month = start_Month, 
                             start_Year = start_Year, 
                             end_Month = end_Month, 
                             end_Year = end_Year)

# explore or manipulate the data as needed, (view the data)
head(romo_data)
head(ever_data)
head(thro_data)
```
...
# Pull in visitation data for each park
romo_data <- unit_visitation(unit = "ROMO", start_Year = start_Year, end_Year = end_Year)
ever_data <- unit_visitation(unit = "EVER", start_Year = start_Year, end_Year = end_Year)
thro_data <- unit_visitation(unit = "THRO", start_Year = start_Year, end_Year = end_Year)

# To explore or manipulate the data as needed (view the data)
head(romo_data)
head(ever_data)
head(thro_data)
...
# For loop
output_floop <- vector("list", length = length(park_units))

for(i in 1:length(park_units)) {
  output_floop[[i]] <- unit_visitation(unit = park_units[i], start_Year = "1980", end_Year = "2021")
}

# Combine data frames
multi_years <- bind_rows(output_floop)
...
```{r}
# Function to fetch visitation data
unit_visitation <- function(unit, start_Month = "01", start_Year, end_Month = "12", end_Year) {
  url <- paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unit,
                "&startMonth=", start_Month,
                "&startYear=", start_Year,
