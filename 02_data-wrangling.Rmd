...
```{r}
unit_visitation <- function(park,startMonth = 1, endMonth = 12, startYear, endYear){
  
  park_visit <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", park, "&startMonth=",startMonth,"&startYear=",startYear,"&endMonth=", endMonth,"&endYear=", endYear)) %>%
...
annual_visitation <- annual_visitation %>%
  mutate(groups = if_else(UnitCode == "Parkwide", "Parkwide", "Individual Parks"))

#plot
plot <- ggplot(data = annual_visitation, aes(x = Year, y = RecVisitation, color = UnitCode)) +
  geom_point() +
  geom_path() + 
  facet_wrap(~groups, scales = "free_y") +
  theme_bw(base_size = 12) +
  labs(title = "Annual Visitation",
       x = "Year",
...
```{r}
# `wide_data`
wide_data_frac <- wide_data %>%
  mutate_at(vars(-Year, -Parkwide), ~./Parkwide * 100)

long_data_frac <- wide_data_frac %>%
  pivot_longer(cols = -Year, values_to = "percent_visitation", names_to = "UnitCode")


# plot
p <- ggplot(filter(long_data_frac, UnitCode != "Parkwide"), 
            aes(x = Year, y = percent_visitation, fill = UnitCode)) +
  geom_col(position = "stack") +
  scale_y_continuous(labels = scales::percent) + 
  theme_minimal() +
  labs(title = "Park Unit Visitation as Percent of Total",
       x = "Year", 
       y = "Percent of Total Visitation")
ggplotly(p)
```
...
joined_data_annual <- joined_data %>%
  ...
  summarize(visits = sum(RecreationVisitors.x), 
            total = sum(RecreationVisitors.y))


joined_data_annual <- joined_data_annual %>%
  mutate(percent = visits/total * 100)


#plot 
ggplot(joined_data_annual, 
            aes(x = Year, y = percent, fill = UnitCode.x)) +
  geom_col(position = "stack") +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Annual Percent Visitation by Park Unit",
       x = "Year",
       y = "Percent of Total Visits")
ggplotly(p)
```
...
long_data_avg <- long_data %>%
  ...
  group_by(Park) %>%
  summarize(avg_visits = mean(RecVisitation))

# plot
ggplot(long_data_avg %>% filter(Park != "Parkwide"),
       aes(x = reorder(Park, avg_visits), y = avg_visits)) +
  geom_col() +
  labs(title = "Average Annual Park Visitation 1990-2021",
       x = "Park",
       y = "Average Annual Visits") +
  theme_minimal() +
  coord_flip()
```
GRSM has the highest vistiation (~10,000,000 people). ACAD has the least visitation (2,500,000 people). 
