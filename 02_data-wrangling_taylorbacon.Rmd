...
```{r}
annual_visitation_grouped <- annual_visitation %>%
  mutate(group = if_else(UnitCode == "Parkwide", "parkwide", "parks"))

plotly::ggplotly(
  ggplot(data = annual_visitation_grouped, aes(x=Year, y=RecVisitation, color = UnitCode))+
    geom_point()+
    geom_path()+
...
plotly::ggplotly(
  ggplot(data = filter(long_data_frac, UnitCode != "Parkwide"))+
    geom_col(aes(x=Year, y=percent_visitation, fill = UnitCode), position = "stack")
)
```
...
joined_data_annual <- joined_data %>%
  ...
  summarize_at(vars(contains("RecreationVisitors")), sum)%>%
  mutate(percent = RecreationVisitors.x/RecreationVisitors.y * 100)

plotly::ggplotly(
    ggplot(data = joined_data_annual)+
    geom_col(aes(x=Year, y=percent, fill = UnitCode.x), position = "stack")
)
...
long_data_avg <- long_data %>%
  ...
  group_by(Park)%>%
  summarize(RecVisAvg = mean(RecVisitation))

long_data_avg %>%
  filter(Park != "Parkwide")%>%
  ggplot()+
  geom_col(aes(x=Park, y=RecVisAvg))+
  labs(title = "Average Park Visitation 1990 to 2021")
```

Great Smoky Mountains National Park on average has the most visitation from 1990 to 2021. Acadia National Park on average has the least visitation from 1990 to 2021.
