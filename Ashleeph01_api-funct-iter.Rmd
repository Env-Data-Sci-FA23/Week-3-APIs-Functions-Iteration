...
parkwide_visitation <- function(year= 2021){
  packed_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/total/",year))
  unpacked_data <- httr::content(packed_data, as = "text", encoding = "UTF-8") 
  final_data <- jsonlite::fromJSON(unpacked_data)
  return(final_data)
}
test1980<- parkwide_visitation(year=1980)
test1999 <- parkwide_visitation(year=1999)
test2018 <- parkwide_visitation(year=2018)
```
...
unit_visitation <- function(unit,start_month, start_year, end_month,end_year){
  url<-httr::GET(url =paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unit,"&startMonth",start_month,"&startYear=", start_year, "&endMonth=",end_month, "&endYear=",end_year))
  data_messy<- httr::content(url,as="text",encoding ="UTF-8")
  data_clean<-jsonlite::fromJSON(data_messy)
  return(data_clean)
}
test2<-unit_visitation(unit="ROMO",start_month = "01",start_year = "2010",end_month ="12",end_year = "2021")
```
...
```{r}
pull_ROMO <- unit_visitation(unit="ROMO",start_month = "11",start_year = "1992",end_month ="12",end_year = "2021")

pull_EVER <- unit_visitation(unit="EVER",start_month = "11",start_year = "1992",end_month ="12",end_year = "2021")

pull_THRO <- unit_visitation(unit="THRO",start_month = "11",start_year = "1992",end_month ="12",end_year = "2021")
...
```{r}
unit_visitation1 <- function(unit,start_month="1", start_year, end_month="12",end_year){
  url<-httr::GET(url =paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unit,"&startMonth",start_month,"&startYear=", start_year, "&endMonth=",end_month, "&endYear=",end_year))
  data_messy<- httr::content(url,as="text",encoding ="UTF-8")
  data_clean<-jsonlite::fromJSON(data_messy)
  return(data_clean)
...
testR1<- unit_visitation1(unit="ROMO",start_year = "1980",end_year = "2021")
testE1 <- unit_visitation1(unit="EVER",start_year = "1980",end_year = "2021")
testT1 <- unit_visitation1(unit="THRO",start_year = "1980",end_year = "2021")
```
...
for(i in 1:length(units)){
  output_floop[[i]] <-
    unit_visitation1(unit = units[i],start_year = 1980,end_year = 2021)}
  multi_units <- dplyr::bind_rows(output_floop)
