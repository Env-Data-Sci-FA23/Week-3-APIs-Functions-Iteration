...
```{r}
raw_nps_visitation_data <- httr::GET(url =paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=01&endYear=2021"))

glimpse(raw_nps_visitation_data)

```

```{r}
unpacked_nps_visit_data <- httr::content(raw_nps_visitation_data, as = "text", encoding = "UTF-8") 
```

```{r}
final_nps_visit_data <- jsonlite::fromJSON(unpacked_nps_visit_data)
```
...
test <- unit_visitation(unit= "SHEN",
                        start_month = "01",
                        start_year = "2000",
                        end_month = "12",
                        end_year = "2020")
...
```{r}
ROMO_data_1992_2021 <- unit_visitation(unit="ROMO",
                        start_month = "11",
                        start_year = "1992",
                        end_month = "12",
                        end_year = "2021")

EVER_data_1992_2021 <- unit_visitation(unit="EVER",
                        start_month = "11",
                        start_year = "1992",
                        end_month = "12",
                        end_year = "2021")

THRO_data_1992_2021 <- unit_visitation(unit="THRO",
                        start_month = "11",
                        start_year = "1992",
                        end_month = "12",
                        end_year = "2021")
...
```{r}
unit_visitation <- function(unit, start_month="01", start_year="1980", end_month="12", end_year="2021"){
...
```{r}
unit_visitation(unit= "ROMO",)
            

unit_visitation(unit= "THRO",)
            

unit_visitation(unit_visitation(unit= "EVER",)
...
```{r}
park <- c("ROMO","EVER","THRO")

```

```{r}
output_flooper <- vector("list", length = length(park))
```
...
output_super <- bind_rows(output_flooper)
```
...
```{r}
park_namesss<- c("ROMO","EVER","THRO")
```

```{r}
map_output <- park_namesss %>%
 map(~ unit_visitation(unit= .))
map_output<- bind_rows(map_output)
