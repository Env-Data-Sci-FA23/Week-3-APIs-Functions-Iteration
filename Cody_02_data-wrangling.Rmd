...
... except, the rows in `parkwide`'s UnitCode and Unit\*Name\* columns are empty. 😑 Let's fix the `UnitCode` column to list "Parkwide" using `mutate()` and an `ifelse()` statement:

```{r}
visitation <- visitation %>% 
  mutate(UnitCode = if_else(is.na(UnitCode), "Parkwide", UnitCode))
glimpse(visitation)
```
...
```{r}
annual_viz <- annual_visitation %>%
  mutate(ParkWide = ifelse(UnitCode == "Parkwide", 'Total_Parks','Indy_Parks'))
#create a column that separates all parks from just 1 park 

plotly::ggplotly(
  ggplot(data=annual_viz) +
    geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    scale_y_continuous(labels = scales::label_scientific()) +
...
```{r}
long_data_percent <- mutate(long_data, percent_vis = TotalVisitation/Parkwide)
```

```{r}
plotly::ggplotly(
  ggplot(data=long_data_percent)+
    geom_point(aes(x=Year, y=percent_vis, color=Park))
)
```
...
joined_percent <- joined_data%>%
  ...
  summarise(annual_park_viz = sum(RecreationVisitors.x),
            annual_total = sum(RecreationVisitors.y)) %>%
  mutate(yearly_avg = annual_park_viz/annual_total)
```

```{r}
plotly::ggplotly(
  ggplot(data=joined_percent)+
    geom_point(aes(x=Year, y=yearly_avg, color=UnitCode.x))
)
```
...
```{r}
Most_Viz <- annual_visitation%>%
  filter(Year>=1990)%>%
  group_by(UnitCode)%>%
  summarise(sum(RecVisitation))
```

The park with the most visitation is GRSM, the least is ACAD.
