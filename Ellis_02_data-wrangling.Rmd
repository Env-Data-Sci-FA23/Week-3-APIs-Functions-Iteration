...
unit_visitation <- function(unitCodes, startYear, endYear, startMonth = "1", endMonth = "12") {
  ...
  visitation_data_raw <- GET(url)
  visitation_data_unpacked <- content(visitation_data_raw, as = "text", encoding = "UTF-8") 
  visitation_data_final <- fromJSON(visitation_data_unpacked)

  return(visitation_data_final)
}
...
units <- parks %>% 
  map(~ unit_visitation(unitCode = ., startYear = start_year, endYear = end_year))
```
...
```{r}
units <- annual_visitation %>% filter(UnitCode != "Parkwide")

each_unit <- ggplotly(ggplot(data=units) +
    geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    scale_y_continuous(labels = scales::label_scientific()) +
    facet_wrap(~UnitCode, scales = "free_y") +
...
park_wide_data <- annual_visitation %>% filter(UnitCode == "Parkwide")

park_wide <- ggplotly(ggplot(data=park_wide_data) +
    geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    scale_y_continuous(labels = scales::label_scientific()))
  

interactive <- subplot(each_unit, park_wide, nrows = 2)
...
```{r}
allparks <- c("ROMO", "YELL", "ACAD", "GRCA", "GRSM", "LAKE", "OLYM", "ZION")

total_vis <- wide_data$Parkwide

unit_percent <- wide_data %>% mutate_at(.vars = allparks, .funs = ~(./total_vis*100))

unit_percent <- unit_percent %>% select(-Parkwide)
view(unit_percent)

unit_per_long <- unit_percent %>% pivot_longer(cols = -Year, names_to = "ParkUnit", values_to = "RecreationVisitors")

per_vis <- ggplotly(ggplot(data=unit_per_long) +
    geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    labs(x = "Year", y = "Park Visitation (% of total)"))
...
```{r}
#Rerunning this code because I iterated over the units and parkwide variables 
units_data <- parks %>% 
  map(~ unit_visitation(unitCode = ., startYear = start_year, endYear = end_year))

#transforming units from a list into a dataframe
units_dataf <- data.frame(units_data)

parkwide_data <- years %>% 
  map(~ parkwide_visitation(year = .x)) %>% 
  bind_rows()

joined_data <- inner_join(x = units_dataf, y = parkwide_data, by = c("Year","Month"))
```
...
data.frame(joined_data)

joined_data_filtered <- joined_data %>% select(contains("UnitCode"), contains("RecreationVisitors"), Year) %>% select(-contains("Non"), -UnitCode.y, -RecreationVisitors.y)

pivoted <- joined_data_filtered %>% pivot_longer(cols = -Year, names_to = , values_to = )
...
```{r}
data.frame(long_data)

grouped <- long_data %>% filter(Year > 1989) %>% group_by(Park) %>% 
...
# Great Smoky Mountains NP has the most visitation, with an annual average of 9.91 million visitors per year (from 1990 to 2021). 

#Acadia NP has the least visitor, wiht an annual average of 2.64 million visitors per year (from 1990 to 2021).
```
