...
units <- UNITS %>%
  map(~ unit_visitation(unit = .x, start_month = "01", start_year = "1980", end_month = "12", end_year = "2021")) %>%
  bind_rows()
```
...
```{r}
 percent_parkwide <- wide_data %>%
    relocate(Parkwide, .after = ZION) %>%
   relocate(Year, .after = Parkwide)%>%
   mutate_all(~ . /Parkwide)
...
```{r}
Percent_long <- percent_parkwide %>%
  pivot_longer(cols = -Year,
               names_to = "Park",
               values_to = "RecVisitations")
```

```{r}
plotly::ggplotly(
  ggplot(data=Percent_long) +
    geom_point(aes(x = Year, y = RecVisitations, color = Park)) +
    geom_path(aes(x = Year, y = RecVisitations, color = Park)) +
    scale_y_continuous(labels = scales::label_scientific()) +
...
```{r}
joined_data <- joined_data %>% mutate(UnitCode.y = if_else(is.na(UnitCode.y), "Parkwide", UnitCode.y))
```

```{r}
merged <- merge (units, parkwide, by = "UnitCode") #nope
```
#I've been struggling with #4 for a few hours. I can't figure out how to combine the parkwide and individual data into combined columns, so I can summarize by year and do the rest. Sorry. 
...
UNITS <- c("ROMO", "ACAD", "LAKE", "YELL", "GRCA", "ZION", "OLYM", "GRSM")
units3 <- UNITS %>%
  map(~ unit_visitation(unit = .x, start_month = "01", start_year = "1990", end_month = "12", end_year = "2021")) %>%
  bind_rows()
```

```{r}
annual_visitation5 <- units3 %>%
  group_by(UnitCode) %>% 
  
  summarize(RecVisitation = sum(RecreationVisitors + NonRecreationVisitors))

annual_visitation5

#Great Smokey Mountains wins with a nonrec and rec combined total od 667680189 for the time period
```
