...
units <- units %>% 
  map(~ unit_visitation1(unit = ., start_year = 1980, end_year = 2021)) %>%
bind_rows()
```
...
annual_visitation2<-annual_visitation %>%
mutate(parkwide= if_else(UnitCode == "Parkwide","park","unit"))
```

```{r}
plotly::ggplotly(
  ggplot(data=annual_visitation2) +
  geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
  geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
  scale_y_continuous(labels = scales::label_scientific()) +
...
```{r}
wide_data$total_visitation <- rowSums(wide_data[, -1])
wide_data[, -1] <- wide_data[, -1] / wide_data$total_visitation * 100
```

```{r}
long_data <- tidyr::gather(wide_data, key = "park_unit", value = "percentage", -Year, -total_visitation,-Parkwide)
```

```{r}
ggplotly(
  ggplot(long_data, aes(x = Year, y = percentage, color = park_unit)) +
    geom_line() +
    labs(title = "Annual Percentage of Total Visitation by Park Unit",
         x = "Year",
...
joined_data$total_visitation <- rowSums(joined_data[, -1])

joined_data[, -1] <- joined_data[, -1] / joined_data$total_visitation * 100
```

```{r}
library(dplyr)

# Assuming you have the columns "Year" and "Month" in both data frames
joined_data <- inner_join(x = units, y = parkwide, by = c("Year", "Month"))

# Convert the columns to numeric
joined_data[, -c(1, 2)] <- sapply(joined_data[, -c(1, 2)], as.numeric)

# Calculate rowSums and store it in a new column
joined_data$total_visitation <- rowSums(joined_data[, -c(1, 2)])

# Calculate percentages and store them in new columns
joined_data[, -c(1, 2)] <- sapply(joined_data[, -c(1, 2)], function(x) x / joined_data$total_visitation * 100)

# Now, you have new columns with the percentages

```


```{r}
long_data <- tidyr::gather(joined_data, key = "park_unit", value = "percentage", -Year, -total_visitation)
```
...
joined_data2 <- joined_data %>%
  ...
  summarise(AnnualParkVis=sum(RecreationVisitors.x),AnnualSystem= sum(RecreationVisitors.y)) %>%
