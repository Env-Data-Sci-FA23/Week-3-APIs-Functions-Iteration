...
```{r}
long_data_percent <- mutate(long_data, Percent_Vis = (TotalVisitation/Parkwide)* 100)
```
...
```{r}
joined_data_percent <- joined_data %>%
  group_by(Year, UnitCode.x) %>%
  summarise(AnnualParkVis = sum(RecreationVisitors.x),
            AnnualSystems = sum(RecreationVisitors.y)) %>% 
              mutate(Annual_Percent = (AnnualParkVis/AnnualSystems)*100)

```

```{r}
ggplotly(
  ggplot(data= joined_data_percent) +
    geom_point(aes(x = Year, y = Annual_Percent, color = UnitCode.x)))
```
...
```{r}
annual_visitation_3 <- annual_visitation %>%
  filter(Year >= 1990) %>%
  group_by(UnitCode) %>%
  summarise(sum(RecVisitation))
```

The park with the most visitation was GRSM (Great Smoky Mountains) and the park with the least visitation was ACAD (Acadia).
