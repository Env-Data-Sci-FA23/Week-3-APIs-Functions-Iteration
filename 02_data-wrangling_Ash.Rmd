...
mapped_units <- park_units %>% 
  map(~unit_visitation(parkUnit = .x, startYear = 1980, endYear = 2021)) %>%
  bind_rows(.id = "park_unit")
...
```{r}
visitation <- bind_rows(parkwide,mapped_units )
```
...
annual_visitation <- annual_visitation %>% 
  mutate(VisitationType = if_else(UnitCode == "Parkwide", "Parkwide", "Individual Parks"))
...
test_long_data <- test_long_data %>%
  mutate(yearly_percentage = ((RecVisitation / Parkwide)*100))

plotly::ggplotly(
 ggplot(data = test_long_data) +
    geom_point(aes(x = Year, y = yearly_percentage, color = Park)) +
    geom_path(aes(x = Year, y = yearly_percentage, color = Park)) +
    scale_y_continuous(labels = scales::label_scientific()) +
    labs(
    y = "Percentage of Total Visitation",
    x = "Year",
    title = "Annual Percentage of Total NPS Visitation by Park Unit"
  ) +
    theme_bw(base_size=10)
)
...
```{r}
joined_data <- inner_join(x = mapped_units, y = parkwide, by = c("Year","Month"))
...
```{r}
joined_data_yearly <- joined_data %>%
  group_by(UnitCode.x, Year) %>%

  summarize(across(contains("RecreationVisitors"), sum, .names = "total_{.col}")) %>%

  mutate(percent = total_RecreationVisitors.x / total_RecreationVisitors.y * 100)


plotly::ggplotly(
  ggplot(data = joined_data_yearly) +
    geom_point(aes(x = Year, y = percent, color = UnitCode.x)) +
    geom_path(aes(x = Year, y = percent, color = UnitCode.x)) +
    scale_y_continuous(labels = scales::label_scientific()) +
    ylab("Total visit in Percent") +
    theme_bw(base_size = 10)
)
...
visit_yearly <- annual_visitation %>%
  filter(Year >= 1990 & Year <= 2021 & UnitCode != "Parkwide") %>%  # Added missing closing parenthesis here
  group_by(UnitCode) %>%
  summarize(Avg_Visit = mean(RecVisitation, na.rm = TRUE))

most_visitation <- visit_yearly %>%
  arrange(desc(Avg_Visit)) %>%
  slice(1)

least_visitation <- visit_yearly %>%
  arrange(Avg_Visit) %>%
  slice(1)

print(most_visitation)
print(least_visitation)


```
As you can see the GRSM (Great Smokey Mountains)  has the most visitation and ACAD (Acadia) has the least visitation. 
