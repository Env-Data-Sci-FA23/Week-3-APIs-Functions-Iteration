...
```{r}
parkwide_visitation <- function(year = 1992){
...
test_1980 <- parkwide_visitation(year = 1980)

View(test_1980)

test_1999 <- parkwide_visitation(year = 1999)

View(test_1999)

test_2018 <- parkwide_visitation(year = 2018)
...
```{r}
RMNP_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=12&endYear=2021"))

  unpacked_data <- httr::content(RMNP_data, as = "text", encoding = "UTF-8")
  final_RMNP <- jsonlite::fromJSON(unpacked_data)
...
unit_visitation <- function(unit, start_month, start_year, end_month, end_year){
  ...
  data_messy <- httr::content(raw_data, as = "text", encoding = "UTF-8")
...
```{r}
unit_visitation_2 <- function(unit, start_month, start_year, end_month, end_year){
...
ROMO_test <- unit_visitation_2 (unit = "ROMO", 
                             start_year = 1980, 
                             end_year = 2021)
...
```{r}
unit <- c("ROMO", "EVER", "THRO")

output_floop <- vector("list", length = length(unit))

for(i in 1:length(unit)){
  
  output_floop[[i]] <-
    unit_visitation_2(unit = unit[i], start_year = 1980, end_year = 2021)
  
}

summary(output_floop)

multi_years_loop <- dplyr::bind_rows(output_floop)
```
...
identical(multi_years_loop, multi_years)
```
