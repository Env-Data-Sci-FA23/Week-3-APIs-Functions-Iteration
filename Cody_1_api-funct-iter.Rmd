...
```{r}
#first pull raw data
raw_eighty <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980")
#next unpack new raw data
unpacked_eighty <- httr::content(raw_eighty, as = "text", encoding = "UTF-8")
#then parse raw data into dataframe
final_eighty <- jsonlite::fromJSON(unpacked_eighty)
```
...
```{r}
#first pull raw data
raw_ninetynine <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1999")
#next unpack new raw data
unpacked_ninetynine <- httr::content(raw_ninetynine, as = "text", encoding = "UTF-8")
#then parse raw data into dataframe
final_ninetynine <- jsonlite::fromJSON(unpacked_ninetynine)
```

```{r}
#first pull raw data
raw_eighteen <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/2018")
#next unpack new raw data
unpacked_eighteen <- httr::content(raw_eighteen, as = "text", encoding = "UTF-8")
#then parse raw data into dataframe
final_eighteen <- jsonlite::fromJSON(unpacked_eighteen)
```
...
raw_ROMO <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=12&endYear=2021")

#unpack raw data
unpacked_ROMO <- httr::content(raw_ROMO, as = "text", encoding = "UTF-8")
...
unit_visitation <- function(unit, start_month, start_year, end_month, end_year){
  url <- httr::GET(url = paste0(
        'https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=', unit,
...
```{r}
ROMO_data <- unit_visitation(unit = 'ROMO', start_month = 11, start_year = 1992, end_month = 12, end_year = 2021)

EVER_data <- unit_visitation(unit = 'EVER', start_month = 11, start_year = 1992, end_month = 12, end_year = 2021)
  
THRO_data <- unit_visitation(unit = 'THRO', start_month = 11, start_year = 1992, end_month = 12, end_year = 2021)
```
...
**For our `unit_visitation()` function, make the default arguments for the start and end months January and December, respectively. This way, we are automatically pulling in data for an entire year. Then, re-run the updated 'unit_visitation()' function for ROMO, EVER, and THRO for the 1980-2021 time period to make sure it works properly.**

```{r}
ROMO_visitation <- unit_visitation(unit = 'ROMO', start_month = 01, start_year = 1980, end_month = 12, end_year = 2021)

EVER_visitation <- unit_visitation(unit = 'EVER', start_month = 01, start_year = 1980, end_month = 12, end_year = 2021)

THRO_visitation <- unit_visitation(unit = 'THRO', start_month = 01, start_year = 1980, end_month = 12, end_year = 2021)
```
...
```{r}
parks_floop <- vector('list', length = length(parks))
```
