...
raw_data_2 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=12&endYear=2021")

glimpse(raw_data_2)
```

```{r}
raw_data_2 # lists 'UTF-8'

# convert content to text
unpacked_data_2 <- httr::content(raw_data_2, as = "text", encoding = "UTF-8")
```

```{r}
final_data_2 <- jsonlite::fromJSON(unpacked_data_2)
...
```{r}
#working on this now... not done/not working!

unit_visitation <- function(unit_code, start_month, start_year, end_month, end_year) {
  raw_data_3 <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unit_code,
                                       "&startMonth=", start_month,
                                       "&startYear=", start_year,
                                       "&endMonth=", end_month,
                                       "&endYear=", end_year))
extracted_data_3 <- httr::content(raw_data_3, as = "text", encoding = "UTF-8") 

final_data_3 <- jsonlite::fromJSON(extracted_data_3)

return(final_data_3)

}

test_3 <- unit_visitation(unit_code = "ACAD", start_month = 06, start_year = 2020, end_month = 05, end_year = 2021)
```
...
```{r}
visitation_rmnp <- unit_visitation(unit_code = "ROMO", start_month = "11", start_year = "1992", end_month = "12", end_year = "2021")

visitation_everglades <- unit_visitation(unit_code = "EVER", start_month = "11", start_year = "1992", end_month = "12", end_year = "2021")

visitation_theodore <- unit_visitation(unit_code = "THRO", start_month = "11", start_year = "1992", end_month = "12", end_year = "2021")
```
...
```{r}
unit_visitation <- function(unit_code, start_month = "01", start_year, end_month = "12", end_year) {
  raw_data_3 <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unit_code,
                                       "&startMonth=", start_month,
                                       "&startYear=", start_year,
                                       "&endMonth=", end_month,
...
```{r}
visitation_rmnp_2 <- unit_visitation(unit_code = "ROMO", start_year = "1980", end_year = "2021")

visitation_everglades_2 <- unit_visitation(unit_code = "EVER", start_year = "1980", end_year = "2021")

visitation_theodore_2 <- unit_visitation(unit_code = "THRO", start_year = "1980", end_year = "2021")
```
...
```{r}
#create a vector
park_units_6 <- c("ROMO", "EVER", "THRO")

#develope empty list for floop
output_floop_6 <- vector("list", length = length(park_units_6))

#create for loop
for(i in 1:length(park_units_6)){
  
  output_floop_6[[i]] <-
    unit_visitation(unit_code = park_units_6[i], start_year = 1980, end_year = 2021)
  
}

summary(output_floop_6)

multi_parks <- dplyr::bind_rows(output_floop_6)
```
...
```{r}
#creating map
output_map_7 <- park_units_6 %>%
  map(~ unit_visitation(unit_code = ., start_year = 1980, end_year = 2021))
#checking that the floop and map have the same outputs
identical(output_floop_6, output_map_7)

multi_parks_7 <- bind_rows(output_map_7)
```
