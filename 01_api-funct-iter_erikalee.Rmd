...
```{r}
#working on this now... not done
unit_visitation <- function(unit_code, start_month, start_year, end_month, end_year) {
  
  raw_data_3 <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes={UNITCODES}&startMonth={STARTMONTH}&startYear={STARTYEAR}&endMonth={ENDMONTH}&endYear={ENDYEAR}", unit_code, start_month, start_year, end_month, end_year))
...
```{r}
#example that chatgbt gave 
unit_visitation <- function(unit_code, start_month, start_year, end_month, end_year) {
  # Construct the URL with placeholders for parameters
  url <- paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes={UNITCODES}&startMonth={STARTMONTH}&startYear={STARTYEAR}&endMonth={ENDMONTH}&endYear={ENDYEAR}")
  
  # Replace the placeholders with actual values
  url <- gsub("{UNITCODES}", unit_code, url)
  url <- gsub("{STARTMONTH}", start_month, url)
  url <- gsub("{STARTYEAR}", start_year, url)
  url <- gsub("{ENDMONTH}", end_month, url)
  url <- gsub("{ENDYEAR}", end_year, url)

  # Send an HTTP GET request to the constructed URL
  raw_data_3 <- httr::GET(url)
  
  # Check if the request was successful
  if (httr::http_error(raw_data_3)) {
    stop("HTTP GET request failed.")
  }
...
```{r}
#test
test_3 <- unit_visitation(unit_code = ACAD)
```
