...
```{r}
parklist <- c("ROMO", "ACAD", "LAKE", "YELL", "GRCA", "ZION", "OLYM", "GRSM")

units <- parklist %>% 
  map(~ unit_visitation(unit = ., start_year = "1980", end_year = "2021"))
map_units <- bind_rows(units)
```
...
annual_visitation <- annual_visitation %>% 
  mutate(group = if_else(UnitCode == "Parkwide", "Parkwide", "All Parks"))
# need help with this ifelse 

plotly::ggplotly(ggplot(data=annual_visitation) +
    geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    scale_y_continuous(labels = scales::label_scientific()) +
...
long_data1 <- long_data1 %>%
  mutate(annual_p = ((RecVisitation / Parkwide)*100))

plotly::ggplotly(ggplot(data = long_data1) +
    geom_point(aes(x = Year, y = annual_p, color = Park)) +
    geom_path(aes(x = Year, y = annual_p, color = Park)) +
    scale_y_continuous(labels = scales::label_scientific()) +
    ylab("Percent Total Visitation")  +
    theme_bw(base_size=10)
)
```
...
```{r}
joined_data <- inner_join(x = map_units, y = parkwide, by = c("Year","Month"))
```
...
```{r}
# group by year and park
joined_data1 <- joined_data %>%
  group_by(UnitCode.x, Year) %>% 
  summarize(TotalRecVis = sum(RecreationVisitors.y), TotalParkRecVis = sum(RecreationVisitors.x))

joined_data1 <- joined_data1 %>%
  mutate(annual_p = ((TotalParkRecVis / TotalRecVis)*100))

plotly::ggplotly(ggplot(data = joined_data1) +
    geom_point(aes(x = Year, y = annual_p, color = UnitCode.x)) +
    geom_path(aes(x = Year, y = annual_p, color = UnitCode.x)) +
    scale_y_continuous(labels = scales::label_scientific()) +
    ylab("Percent Total Visitation")  +
    theme_bw(base_size=10)
)
...
annual_filter <- annual_visitation %>%
  filter(Year >= 1990)

# group by park and summarize to find average
annual_filter1 <- annual_filter %>%
  group_by(UnitCode) %>% 
  summarize(AvgVis = mean(RecVisitation))

# need to drop parkwide row
annual_filter1 <- annual_filter1 [c(-6),]

max(annual_filter1$AvgVis)
# the park with the highest avg visitation is GRSM with 9,907,814

min(annual_filter1$AvgVis)
# the park with the lowest avg visitation is ACAD, with 2,636,266
# Acadia is one of my favorite National Parks and I'm not upset that it's being relatively gatekept from the masses
```
