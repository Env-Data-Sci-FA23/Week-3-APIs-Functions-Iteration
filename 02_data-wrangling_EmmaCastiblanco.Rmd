...
```{r}
unit_visitation <- function(unitCodes="ROMO", startMonth=1, startYear=1980, endMonth=11, endYear=2021){
...
units_1<- c("ROMO", "ACAD", "LAKE", "YELL", "GRCA", "ZION", "OLYM", "GRSM") 

units_2 <-map(units_1, ~unit_visitation(unitCodes=.))
...
```{r}
parkvisitations <- filter(annual_visitation, UnitCode=="Parkwide")

plotly::ggplotly(
 ggplot(data=parkvisitations) +
 geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
  scale_y_continuous(labels = scales::label_scientific()) +
    facet_wrap(~UnitCode, scales = "free_y") +
...
everythingelse <- filter(annual_visitation, UnitCode != "Parkwide")

plotly::ggplotly(
 ggplot(data=everythingelse) +
 geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
  scale_y_continuous(labels = scales::label_scientific()) +
 theme_bw(base_size=10)
...
wide_data2[, -which(names(wide_data2) == "Year")] <- wide_data2[, -which(names(wide_data2) == "Year")] / wide_data2$Parkwide
wide_data2[, -which(names(wide_data2) == "Year")] <- wide_data2[, -which(names(wide_data2) == "Year")]*100


wide_data3 <- wide_data2[, -which(names(wide_data2) == "Parkwide")] %>%
  gather(wide_data3, y, -Year)

plotly::ggplotly(
  ggplot(wide_data3, aes (x=Year, y=y, color= wide_data3)) +
  geom_line() +
  labs(
    title= "Visitors by Each Park",
    x = "Year",
    y= "Percentage",
    color="Park"
  ) +
...
```{r}
annual_sum<- joined_data %>%
  group_by(Year, UnitCode.x) %>%
  summarize(unit_annual=sum(RecreationVisitors.x), parkwide_annual=sum(RecreationVisitors.y))

annual_sum<-
  mutate(annual_sum, percentage_units= (unit_annual/parkwide_annual)*100)

plotly::ggplotly(
  ggplot(annual_sum, aes (x=Year, y=percentage_units, color= UnitCode.x)) +
  geom_line() +
  labs(
    title= "Visitors by Each Park",
    x = "Year",
...
```{r}
  sort(colMeans(wide_data), decreasing=FALSE)
```
