...
```{r}
unit_visitation <- function(year){

# pull in the data
raw_data_ROMO <- httr::GET(url = 
                        paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=12&endYear=2021", year))

# convert content to text
extracted_data_ROMO <- httr::content(raw_data, as = "text", encoding = "UTF-8") 

# parse text from JSON to data frame
final_data_ROMO <- jsonlite::fromJSON(extracted_data)

return(final_data_ROMO)
}
```
...
```{r}
unit_visitation <- function(year){
ROMO_data <- httr::GET(url = 
                        paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=11&startYear=1992&endMonth=12&endYear=2021", year))

EVER_data <- httr::GET(url = 
                        # parse out year so that it can be chosen with the "year" argument, using paste0()
                        paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=11&startYear=1992&endMonth=12&endYear=2021", year))

THRO_data <- httr::GET(url = 
                        # parse out year so that it can be chosen with the "year" argument, using paste0()
                        paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=11&startYear=1992&endMonth=12&endYear=2021", year))
...
```{r}
unit_visitation <- function(month = "January-December"){
ROMO_data <- httr::GET(url = 
                        paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=1980&endMonth=12&endYear=2021", month))
...
```{r}
output_map <- map(units, unit_visitation)
```
