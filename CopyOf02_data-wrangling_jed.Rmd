...
```{r}
output_flooper <- vector("list", length = length(parks))
```
...
for(i in 1 :length(parks)){
  ...
  output_flooper[[i]] <-
    unit_visitation(unit = parks[i])
}

units <- bind_rows(output_flooper)
```
...
```{r}
visitors <- annual_visitation %>% 
  mutate(category=if_else(UnitCode=="Parkwide", "park", "unit"))
```
...
pivot_longer(cols = -c(Year, Parkwide),
             names_to = "Park",
             values_to = "visitors") %>%
  mutate(percent=visitors/Parkwide*100)
...
npso<- joined_data %>%
  ...
  summarise(totalvistorsunit=sum(RecreationVisitors.x), totalvisitorspark= sum(RecreationVisitors.y))%>%
  mutate(percent=totalvistorsunit/totalvisitorspark*100)
```
