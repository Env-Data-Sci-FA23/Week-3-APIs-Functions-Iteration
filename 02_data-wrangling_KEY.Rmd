---
title: "3.2: Data Wrangling and Visualization KEY"
author: "Katie Willi"
date: "`r Sys.Date()`"
...
or <- wide_data %>%
  pivot_longer(cols = -c(Year,Parkwide), names_to = "Park", values_to = "TotalVisitors") %>%
  mutate(Percentage = 100*(TotalVisitors/Parkwide))
...
other_way <- yearly %>%
  filter(Year >= 1990) %>%
  group_by(UnitCode) %>%
  summarize(mean = mean(RecVisitation))
```
