...
```{r}
#create a vector
parkunits_1 <- c("ROMO", "ACAD", "LAKE", "YELL", "GRCA", "ZION", "OLYM", "GRSM")
#mapping
output_map_1 <- parkunits_1 %>%
  map(~ unit_visitation(unit_code = ., start_year = 1980, end_year = 2021))
#binding rows and final output
units <- bind_rows(output_map_1)
```
...
```{r}
annual_visitation_edit <- annual_visitation %>%
  mutate(visitations = if_else(UnitCode == "Parkwide", "parkwide", "park"))

plotly::ggplotly(
  ggplot(data=annual_visitation_edit) +
  geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
  geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
  scale_y_continuous(labels = scales::label_scientific()) +
  facet_wrap(~visitations, scales = "free_y") +
  theme_bw(base_size=10)
)
```
...
```{r}
 long_data_2<- wide_data %>%
   pivot_longer(cols = -c(Parkwide,Year),
               names_to = "Park",
               values_to = "RecVisitation") %>%
   #creating a column with the total percentage of parkwide visitiation per park, per year
...
joined_data_2 <- joined_data %>%
  mutate(monthly_total = NonRecreationVisitors.y + RecreationVisitors.y) %>%
  mutate(park_total = NonRecreationVisitors.x + RecreationVisitors.x)%>%
  mutate(percentage = (park_total/monthly_total)*100) %>%
  select(-c(monthly_total, park_total, Month, NonRecreationVisitors.x, NonRecreationVisitors.y, RecreationVisitors.x, RecreationVisitors.y, UnitCode.y, UnitName.y, UnitName.x)) %>%
  group_by(UnitCode.x, Year) %>%
  summarize(annual_total = sum(percentage))
```

```{r}
#creating the time series plot - this is total visitations, included recreational and non-recreational!
 plotly::ggplotly(
  ggplot(data=joined_data_2) +
  geom_point(aes(x = Year, y = annual_total, color = UnitCode.x)) +
  geom_path(aes(x = Year, y = annual_total, color = UnitCode.x)) +
  scale_y_continuous() +
  theme_bw(base_size=10) + labs(y = "Percentage")
)
```
...
```{r}
#creating an average park visitation dataset
avg_park_visit_months <- long_data_2 %>%
  filter(if_all(.col = Year,~ !(.x < 1990))) %>%
  group_by(Park, Year) %>%
  summarize(total_visitations = sum(RecVisitation))

#getting the average of highest visitation park from years 1990-2021
ACAD_avg <- avg_park_visit_months %>%
  filter(Park == "ACAD") %>%
  summarise(mean_total_visitations = mean(total_visitations, na.rm = TRUE))

GRSM_avg <- avg_park_visit_months %>%
  filter(Park == "GRSM") %>%
  summarise(mean_total_visitations = mean(total_visitations, na.rm = TRUE))
```
...
On average, from the years 1990-2021 Great Smokey National Park has the highest visitations with an average of 9,907,814 visits per year.

Acadia National Park has the average lowest visitation, with an average of 2,636,266 visits per year for the years 1990-2021.
