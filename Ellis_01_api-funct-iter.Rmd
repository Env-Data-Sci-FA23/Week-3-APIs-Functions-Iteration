...
```{r}
raw_data <- GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1992")
...
```{r}
#1980 data
raw_data_1980 <- GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980")
...
```{r}
visitation_data_raw <- GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=1&startYear=2010&endMonth=12&endYear=2021")
...
test <- unit_visitation("ROMO", 1, 2008, 12, 2010)
test
...
```{r}
pull_ROMO <- unit_visitation ("ROMO", 11, 1992, 12, 2021)
...
pull_ROMO <- unit_visitation ("ROMO", 1980, 2021)
pull_EVER <- unit_visitation ("EVER", 1980, 2021)
pull_THRO <- unit_visitation ("THRO", 1980, 2021)
...
output_floop_unit <- vector("list", length = length(parks))

for(i in 1:length(parks)){
  
  output_floop_unit[[i]] <-
    unit_visitation(unitCode = parks[i], start_year, end_year)
  
}

multi_park <- bind_rows(output_floop_unit)
multi_park
```
...
```{r}
identical(output_floop_unit, output_map)
```
...
multi_park <- bind_rows(output_map_parks)
